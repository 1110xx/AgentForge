...
| observability/ | 目录 | Phase 4.3（G5）：OTLP collector 接线（属性脱敏→Tempo/Prometheus）、告警规则集（correctness+SLO+业务，CRD 与 rule_files 双形态）、最小观察栈（Loki/Tempo/Prometheus/Grafana/promtail）+ 3 块看板 + README |
...
- **可观测门（Phase 4.3 G5，2026-08-23 交付）**：scripts/test-observability.sh --stack-only / 全量——OTLP collector（otel/opentelemetry-collector-contrib）→ Tempo（traces）/ Prometheus（metrics），promtail → Loki（JSON 结构化日志，trace_id/run_id/attempt_id 关联），Grafana 三看板（总览/单次 Run 追踪/模型成本容量）；告警集 correctness 8 + SLO 2 + 业务 5（PrometheusRule CRD + rule_files 双形态）；helm values `observability` 段接线（api/orchestrator 注入 OTLP 端点 + JSON 日志，Runner Pod 由 worker 透传）；后端单元测试 21 条（tests/test_telemetry_wiring.py）全绿。
